...
function promiseQueue(promises, maxConcurrent) {
  let index = 0; // 当前处理的promise的索引
  const results = []; // 存储所有promises的结果
  let running = 0; // 当前正在执行的任务数
  const taskQueue = promises.slice(); // 复制一份promises数组作为任务队列

  // 返回一个新的Promise，确保所有任务都能按照并发要求执行完毕
  return new Promise((resolve, reject) => {
    // 定义一个执行任务的函数
    const runTask = () => {
      // 如果任务都完成了，就解析最终的Promise
      if (taskQueue.length === 0 && running === 0) {
        resolve(results);
        return;
      }

      // 当还有任务等待执行且未达到最大并发限制时，执行任务
      while (running < maxConcurrent && taskQueue.length > 0) {
        const currentTaskIndex = index++; // 获取当前任务的索引，并准备下一个索引
        const task = taskQueue.shift(); // 获取任务队列中的下一个任务
        ...
        task()
          .then((result) => {
            ...
            running--; // 完成一个任务，正在运行的任务数减1
            runTask(); // 尝试执行下一个任务
          })
          .catch((err) => {
            reject(err); // 如果任务失败，直接拒绝整个promiseQueue的结果
          });
      }
    };
